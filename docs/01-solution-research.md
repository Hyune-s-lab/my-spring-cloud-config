...
현재 요구사항에는 **OpenBao를 영속성/audit 저장소로 사용하고, Spring 서비스는 Spring Cloud Config Server의 Vault backend를 활용하며, 프론트는 얇은 projection API로 제공하는 구조**가 가장 적합합니다.

- OpenBao는 KV v2 versioning, audit device, policy, HTTP API, Web UI를 제공합니다.
- Spring Cloud Config Server는 Vault backend로 OpenBao를 조회해 Spring 서비스용 설정 API를 제공합니다.
- Common Config Server의 수동 구현 범위는 feature flag/config 검증과 프론트용 projection API로 제한합니다.
...
| Spring Cloud Config        | ✅                     | ✅ HTTP API<br>✅ Spring client    | ❌  | ⚠️ Vault backend 사용                    | ✅ Actuator/Bus 확장     | Vault/OpenBao backend, Spring client, refresh 흐름을 공식 기능으로 제공합니다.                      | 관리자 UI와 feature flag projection은 제공하지 않으므로 프론트용 facade는 별도로 필요합니다.                                 |
...
- feature flag 제품은 아니므로 schema 검증, entitlement, 프론트 projection API는 common-config 서버가 담당해야 합니다.
- 프론트에 OpenBao를 직접 노출하기보다 common-config 서버가 읽기 전용 projection을 제공하는 편이 안전합니다.
...
- 프론트/비 Spring 서비스는 별도 REST projection API와 polling/ETag/SSE 방식이 필요합니다.
